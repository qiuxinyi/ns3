...
TypeId GenQueueDisc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::GenQueueDisc")
                      ...
                      .AddTraceSource ("getPrediction", "callback to get predictions",
                                       ...
                                       "ns3::Packet::TracedCallback")
                      .AddTraceSource ("getCcRecord", "callback to get predictions",
                                       MakeTraceSourceAccessor (&GenQueueDisc::m_getCcRecord),
                                       "ns3::Packet::TracedCallback")
...
bool GenQueueDisc::MyCCBufferManagement(uint32_t priority, Ptr<Packet> packet) {
  ...
  uint32_t mycc = 3;
  MyCcTag ccTag;
  ccfound = packet->PeekPacketTag(ccTag);
  if (ccfound){
    mycc = ccTag.GetCc();
  }
  ...
  if (time-preTime>0.1){
        ...
        //     if (found){     
        //       uint32_t old_pri=a.GetPriority();
        //       a.SetPriority(priority);
        //       uint32_t new_pri=a.GetPriority();
        //       //std::cout <<"old pri: "<<old_pri<<" new pri: "<<new_pri<<" priority: "<<priority<<std::endl;
        //     }
        //   }
        // }
  ...
  ccAcceptQueueLength[portId][priority][mycc]+=packet->GetSize();
  ...
  if(c>MyM) {
    ...
    if(currentSize>queueSize){
      ...
      remaining = queueSize-currentSize;
      //std::cout << "cubic remaing: "<< remaining <<" queueSize: "<<queueSize<<" currentSize: "<<currentSize<<" portBW: "<<portBW<<currentSize<<" totalNum: "<<totalNum<< std::endl;
    }
    ...
    if(currentSize>queueSize){
      ...
      remaining = queueSize-currentSize;
      //std::cout << "remaing: "<< remaining <<" queueSize: "<<queueSize<<" currentSize: "<<currentSize<<" portBW: "<<portBW<<currentSize<<" totalNum: "<<totalNum<< std::endl;
    }
  }
...
bool GenQueueDisc::AcceptPacket(uint32_t priority, Ptr<Packet> packet, Ptr<QueueDiscItem> item) {
  bool accept;
  int drop = 0;
  //std::cout << "AcceptPacket: " <<bufferalg << std::endl;
  switch (bufferalg) {
  case DT:
  ...
  case CREDENCE:
    ...
    break;
  case MYCC:
    accept = MyCCBufferManagement(priority, packet);
    break;
  default:
    accept = DynamicThresholds(priority, packet);
  }
...
GenQueueDisc::DoEnqueue (Ptr<QueueDiscItem> item)
{
  ...
  if (!sharedMemory->myEnqueueBuffer(item->GetPacket()->GetSize(), portId, p,mycc)) {
    DropBeforeEnqueue (item, LIMIT_EXCEEDED_DROP);
